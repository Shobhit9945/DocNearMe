...
const formatAuthError = (value) => {
  if (!value) return 'Unable to sign in'

  const map = {
    CredentialsSignin: 'Invalid email or password',
    'Please sign in with Google for this account': 'Please sign in with Google for this account',
    ...
    try {
      ...
      if (result?.error) {
        if (result.error === 'Configuration') {
          setError('Google sign-in is not configured. Please check the setup documentation.')
        } else {
      ...
      setError('Google sign-in is not available. Please check the configuration.')
    }
  }
